...
class Feeder(threading.Thread):
    ...
    def run(self):
        ...
        for _ in range(FOOD_TO_MAKE):
            ...
            item = self.table_queue.put(Food())
            self.table_lock.release()
            self.food_made += 1
            print(f'Food made: {self.food_made}')
...
class Eater(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            print(f'Sem can i eat value: {self.sem_can_I_eat._value}')
            self.sem_can_I_eat.acquire()
            #print(f'Here is the Sem can i eat aquire: ')
            # TODO - lock, get food from queue, and unlock
            self.table_lock.acquire()
            item = self.table_queue.get()
            #print(f'Here is the Item: {item}')
            self.table_lock.release()
            # TODO - if item from queue is None, then break; else increment food ate counter
            if item == None:
                print('Break here') 
                break
            else: self.food_eaten += 1

...
def draw(tur: SlowTurtle, main_turtle: RawTurtle):
    ...
    # TODO - modify to make these draw in threads and not draw in order 
    # (meaning, that it shouldn't draw all the squares, then the circles, etc.). 
    # It might draw 4 of the same shapes at a time, but then it should draw a different shape. 
    # For advanced users, try and see if you can figure out a way for it to not draw 4 of the same shape, 
    # but draw a different shape each time (hint: random module).
    lock = threading.Lock()
    t1 = threading.Thread(draw_squares(tur, lock))
    t1.start()
    t2 = threading.Thread(draw_circles(tur, lock))
    t2.start()
    t3 = threading.Thread(draw_triangles(tur, lock))
    t3.start()
    t4 = threading.Thread(draw_rectangles(tur, lock))
    t4.start()

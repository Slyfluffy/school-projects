...
def read_thread(queue: mp.Queue, filename: str, cpu_count):
    file = open(filename)
    for line in file:
...
def main():
    ...
    thread = threading.Thread(read_thread(prime_queue, filename, cpu_count))

    # TODO create prime processes
    processes = []
    for _ in range(cpu_count):
        processes.append(mp.Process(prime_process(prime_queue, primes)))
